...
const Team = () => {
  const team = [
    { name: "ThanushMohan Chalamala", role: "Founder",text:"Driving inovation in educational technology with a passion for quantum-speed learning and ai-explorer", img: thanush , ig: "https://www.instagram.com/simply_bittu?utm_source=ig_web_button_share_sheet&igsh=MTBuNmxieG05aTE3OA==", linkedin: "https://www.linkedin.com/in/thanush-mohan-464184354?utm_source=share_via&utm_content=profile&utm_medium=member_android" },
    ...
    { name: "Vamsi Gedela", role: "Cyber Security Analyst",text:"Securing digital platforms, analyzing threat vectors, and safeguarding organizational assets.", img: vamsi, ig: "https://www.instagram.com/vamsivasudev9?igsh=MXQza2tnYjE0MzNqaw==" },
  ];
  return (
          ...
          <div className="mt-8 grid sm:grid-cols-2 md:grid-cols-3 gap-6">
            {team.map((m, i) => (
              <motion.div key={m.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}
                className="group relative rounded-2xl border border-black/10 dark:border-white/10 bg-white/70 dark:bg-white/5 p-6 text-center flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-slate-700 dark:text-white/70 text-lg mt-2">{m.text}</p>
                </div>
                <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
                  <a href={m.ig} target="_blank" rel="noreferrer"
                    className="inline-flex items-center justify-center rounded-full px-3 py-1.5 text-xs font-semibold text-white bg-gradient-to-r from-[#feda75] via-[#d62976] to-[#4f5bd5] transition-transform hover:-translate-y-0.5">
                    <Instagram size={14} className="mr-1" />
                    Instagram
                  </a>
                  {m.linkedin && (
                    <a href={m.linkedin} target="_blank" rel="noreferrer"
                      className="inline-flex items-center justify-center rounded-full px-3 py-1.5 text-xs font-semibold text-white bg-[#0077b5] hover:bg-[#006396] transition-transform hover:-translate-y-0.5">
